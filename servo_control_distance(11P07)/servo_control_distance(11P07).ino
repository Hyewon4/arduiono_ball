...
#define _EMA_ALPHA 0.23    // EMA weight of new sample (range: 0 to 1)
...
void loop() {
  ...
  if (dist_ema <= _TARGET_LOW){
    ...
    myservo.writeMicroseconds(int(round(_DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN) / 180.0 * (dist_ema - _TARGET_LOW)))));
  }
